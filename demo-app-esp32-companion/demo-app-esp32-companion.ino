...
#define CHARACTERISTIC_BUTTON_1 "6E400002-B5A3-F393-E0A9-E50E24DCCA9F"
#define CHARACTERISTIC_BUTTON_2 "6E400003-B5A3-F393-E0A9-E50E24DCCA9F"
#define CHARACTERISTIC_LED      "6E400004-B5A3-F393-E0A9-E50E24DCCA9F"
#define CHARACTERISTIC_LONG_DATA      "6E400005-B5A3-F393-E0A9-E50E24DCCA9F"

//Long piece of data > 23 bytes, works whether or not we've changed max MTU in our app
...
//Even longer piece of data, doesn't work at all 
//std::string longdata = "This is an even longer string of text, because I want to know if we can actually send more data than fits inside a single MTU. This would mean that the Bluetooth stack on both ends supports separating packages in multiple MTU's automatically, and means that we don't have to worry about any nonsense involving data that's too large. We could send entire megabytes if we wanted even though it requires multiple MTUs to send this sort of data. It is super duper large after all and the data within this string is definitely a lot longer than 512 bytes. So yeah, let's see what happens if we try sending this stuff, shall we? All right then.";
//not that long piece of data < 23 bytes
...
class LongCallback: public BLECharacteristicCallbacks {
    void onRead(BLECharacteristic *pCharacteristic) {
      //You can put some data here to send to the device when you receive a request
      pCharacteristic->setValue(longdata);
